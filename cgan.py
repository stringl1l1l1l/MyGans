...
# 加载cup_plus数据集
    pos_data = Cup(path="./data/cup_plus/tensor_data/pos",
    ...
    neg_data = Cup(path="./data/cup_plus/tensor_data/neg",
        transform = transforms.Compose(
            [
            transforms.Resize(opt.img_size),
            transforms.CenterCrop(opt.img_size),
...
def sample_image(n_row, batches_done):
    ...
    # save_image(gen_imgs.data * std + mean, "images/%d.png" % batches_done, nrow=n_row, normalize=True)
...
for epoch in range(opt.n_epochs):
    for i, (imgs, labels) in enumerate(dataloader):
        ...
        valid = Variable(FloatTensor(batch_size, 1).fill_(0.0), requires_grad=False)
